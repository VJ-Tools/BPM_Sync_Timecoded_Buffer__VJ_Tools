...
def test_basic_output():
    ...
    # VACE: frames as list (Scope's preprocess_video handles frame count + normalization)
    assert "vace_input_frames" in result, "vace_input_frames should be present"
    vace_frames = result["vace_input_frames"]
    assert isinstance(vace_frames, list), "vace_input_frames should be a list"
    assert len(vace_frames) == 1, "Should have 1 VACE frame for 1 input frame"
    assert vace_frames[0].shape == (1, 336, 576, 3), f"VACE frame shape wrong: {vace_frames[0].shape}"
    # No masks — VaceEncodingBlock defaults to all-ones (condition on entire frame)
    assert "vace_input_masks" not in result, "Should not send masks (auto-generated by Scope)"

    print("  [OK] Basic output test passed")


def test_vace_frame_list_multi():
    """Test VACE frames as list with multiple input frames."""
    from bpm_sync_timecoded_buffer.pipeline import BpmTimecodedBufferPipeline, BpmBufferConfig

    config = BpmBufferConfig()
    pipeline = BpmTimecodedBufferPipeline(config)

    # Simulate 4-frame chunk
    frames = [make_test_frame() for _ in range(4)]
    result = pipeline(video=frames)

    assert "vace_input_frames" in result
    vace_frames = result["vace_input_frames"]
    assert isinstance(vace_frames, list), "Should be a list"
    assert len(vace_frames) == 4, f"Should have 4 VACE frames, got {len(vace_frames)}"

    # Each frame is (1, H, W, C) float [0, 255]
    for i, f in enumerate(vace_frames):
        assert f.shape == (1, 336, 576, 3), f"Frame {i} shape wrong: {f.shape}"

    print("  [OK] VACE frame list multi test passed")
